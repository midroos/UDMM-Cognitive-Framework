...
# We will re-import Intention from a separate file
...
class UDMM_Agent:
    ...
    def set_goal(self, goal_pos):
        ...
        self.intention.set_goal = goal_pos
    def step_update(self, state_tuple, action, reward, next_state_tuple, done):
        # --- Integration of Prediction Module ---
        # 1. Predict outcome before learning from the real one.
        predicted_reward, _ = self.prediction.predict(state_tuple, action)
        
        # 2. Calculate the real prediction error.
        pe = abs(reward - predicted_reward)
        
        # 3. Update the predictive model with the actual outcome.
        self.prediction.update(state_tuple, action, reward, next_state_tuple)
        # --- End of Integration ---

        agent_pos, goal_pos = state_tuple
        dist = np.linalg.norm(np.array(agent_pos) - np.array(goal_pos))
        
        # Use the real `pe` instead of the placeholder.
        self.emotion.record(reward, pe, dist)

        self.intention.record_observables(reward, dist, novelty=False) # novelty is not used yet
